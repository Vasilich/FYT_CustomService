...
- Поддерживает список целевых запусков при ACCON (сохраняется на диск):
  ...
  - Переключатель enabled (checkbox) для каждой цели, состояние сохраняется.
- В GUI есть:
  - Старт/стоп сервиса.
  ...
  - Настройка fallback-плеера для ACCON, если не найден сохранённый плеер.
...
1. Загрузка сохранённого пакета из ACCOFF.
   - Если сохранённого пакета нет и задан fallback-плеер, запускается fallback-плеер, выдерживается задержка ACCON и отправляется `PLAY`.
...
- В app picker отображаются иконка приложения, имя и package.
...
- Измеренный размер release APK:
  - до оптимизации: `8,010,620` bytes (`7.64 MiB`)
  - после оптимизации с `minSdk 29`: `1,365,341` bytes (`1.30 MiB`)
- Проверен вариант `minSdk 33` для Android 13-only: экономия составила только около `27,864` bytes относительно `minSdk 29`, поэтому `minSdk` оставлен `29`.

### 2026-04-04
- Добавлена настройка fallback-плеера для ACCON на случай отсутствия сохранённого плеера из ACCOFF.
- Fallback-сценарий повторяет стандартный сценарий запуска плеера на ACCON: запуск, ожидание задержки, отправка `PLAY`.
- В диалоге настроек добавлен выбор fallback-плеера через app picker.
- Задокументировано, что app picker показывает иконки приложений.

### 2026-04-14
- В настройках fallback-плеера теперь отображаются **имя приложения + иконка** (вместо package name).
- Добавлен handoff-файл для продолжения работы на другом ПК:
